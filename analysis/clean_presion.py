...
#
# CRITERIOS (LIMPIEZA)
# - Aplica un "plan" CSV generado por find_outliers_presion.py.
# - Preserva estructura de subcarpetas: output-dir / (ruta relativa desde input-dir).
# - Reglas drop:
#   - si (asset_id coincide) AND (value fuera de [value_min, value_max]) dentro de rango de fechas del plan
# - Además:
#   - elimina filas con timestamp inválido
#   - convierte value a numérico (NaN se conserva; no se dropea salvo regla)
#
# EJEMPLOS (PROMPT / COMMAND)
# - Aplicar plan a todos los CSV:
#   python analysis\clean_presion.py --plan reports\cleaning_plan_presion_YYYYMMDD_HHMMSS.csv --input-dir output\datos_def\presion --output-dir output\datos_clean\presion
#
# - Dry-run:
#   python analysis\clean_presion.py --plan ... --input-dir ... --output-dir ... --dry-run
# -----------------------------------------------------------------------------
