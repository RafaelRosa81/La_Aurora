# -----------------------------------------------------------------------------
# find_outliers_presion.py
#
# CRITERIOS (AUDITORÍA / OUTLIERS)
# - Detecta timestamp en: timestamp/ts/datetime/fechahora (case-insensitive).
# - Determina asset_id:
#   - si existe asset_label y NO parece hora/fecha -> usa asset_label
#   - si asset_label parece hora/fecha (contaminado) -> usa fallback (carpeta o prefijo archivo)
# - Detecta la variable "presion" buscando columnas candidatas (ver VALUE_CANDIDATES).
# - Outliers por asset:
#   - valores fuera de [abs_min, abs_max] (si se definen) y/o fuera de [pct_min, pct_max]
#   - saltos anómalos (delta absoluto) por encima de p99 del |delta| (solo auditoría)
# - Genera:
#   - Excel de auditoría (Summary + OutOfRangeRows + Notes)
#   - CSV plan de limpieza (para clean_presion.py) con reglas "drop" por archivo+asset
#
# EJEMPLOS (PROMPT / COMMAND)
# - Auditar TODOS los assets:
#   python analysis\find_outliers_presion.py --input-dir output\datos_def\presion --group presion --all --pct-min 0.5 --pct-max 99.5
#
# - Auditar un asset particular (substring):
#   python analysis\find_outliers_presion.py --input-dir output\datos_def\presion --group presion --asset Principal
# -----------------------------------------------------------------------------
